CREATE OR REPLACE FUNCTION to_minutes (interval) RETURNS int AS '
DECLARE i ALIAS FOR $1; c int; BEGIN c:=0; c:=date_part(''day'',i)*1440; c:=c+date_part(''hours'',i)*60; c:=c+date_part(''minutes'',i); RETURN c; END; ' 
language plpgsql;
...
create or replace view v_kill as
...
create or replace view v_kill_bots as
...
create or replace view v_damage_bots as
...
CREATE OR REPLACE VIEW v_kills_per_time AS
	SELECT k.attacker, count(k.*) AS kills, k.gamesession, to_minutes(s.gamestopped-s.gamestarted) AS timeplayed FROM v_kill k, t_session s
	WHERE s.id = k.gamesession
	GROUP BY k.attacker, k.gamesession, s.gameStarted, s.gamestopped;

create or replace view v_sessionstats_allplayers as
